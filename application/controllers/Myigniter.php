...
class Myigniter extends MY_Controller
{
    ...
    public function login($identity = null, $password = null)
    {
        ...
        if (!$data['features']['disable_all_social_logins']) {
            ...
            $sociallogin = $this->social_login(); // Return facebook, twitter and google login urls array from main controller

            if ($data['features']['login_via_facebook']) {
                $data['facebook'] = $sociallogin['facebook']; // facebook login is used to get facebook Login Url from main controller
            }
            if ($data['features']['login_via_google']) {
                $data['google'] = $sociallogin['google']; // google login is used to get Google Login Url from main controller
            }
            if ($data['features']['login_via_twitter']) {
                $data['twitter'] = $sociallogin['twitter']; // twitter login is used to get twitter Login Url from main controller
            }
            if ($data['features']['login_via_linkedin']) {
                $data['linkedin'] = site_url('myigniter/linkedin_login'); // linkedIn login is used to get linkedIn Login from main controller
            }
        }
     ...
     * @return array
     **/
    public function slug_page_check($post_array, $primary_key = null)
    {
